...
function Admin(){
  ...
  async function createClient(){ const {data}=await supabase.from('clients').insert({name:'מטופל חדש', total_minutes:600, used_minutes:0, purchased_at:today(), expires_at:'2026-12-31'}).select().single(); await load(); if(data?.id) setSelectedId(data.id) }
  ...
  return <Shell>
    ...
    <div className="adminGrid">
      <Card><div className="content"><div className="rowBetween"><h2>מטופלים</h2><Button onClick={createClient}><Plus size={16}/> חדש</Button></div><div className="search"><Search size={16}/><input placeholder="חיפוש" value={query} onChange={e=>setQuery(e.target.value)} /></div>{filtered.map(c=><button className={`clientBtn ${selected?.id===c.id?'active':''}`} key={c.id} onClick={()=>setSelectedId(c.id)}><b>{c.name}</b><small>נותרו {minutesToText(c.total_minutes-c.used_minutes)}</small></button>)}</div></Card>
